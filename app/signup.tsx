      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={20}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
          <View style={styles.formContainer}>
            ...
            {/* input username */}
            <TextInput placeholder="Username" style={styles.inputName} placeholderTextColor="#999" value={username} onChangeText={setUsername} onBlur={validateUsername} />

            {usernameError ? <Text style={{ color: "red", marginTop: -10, marginBottom: 10 }}>{usernameError}</Text> : null}

            {/* input email */}
            <TextInput placeholder="Email" style={styles.inputEmail} placeholderTextColor="#999" value={email} onChangeText={setEmail} onBlur={validateEmail} />

            {emailError ? <Text style={{ color: "red", marginTop: -10, marginBottom: 10 }}>{emailError}</Text> : null}
            ...
            {passwordError ? <Text style={{ color: "red", marginTop: 5, marginBottom: -8 }}>{passwordError}</Text> : null}
            ...
            {confirmPasswordError ? <Text style={{ color: "red", marginTop: 5, marginBottom: 10 }}>{confirmPasswordError}</Text> : null}

            <TouchableOpacity style={styles.button} onPress={handleSignup} disabled={isLoading}>
              {isLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Sign Up</Text>}
            </TouchableOpacity>

            {signupError ? <Text style={{ color: "red", marginTop: 10, marginBottom: 10, textAlign: "center" }}>{signupError}</Text> : null}
